...
describe('Login Route', () => {
  test('Should return 400 if no email is provided', async () => {
    ...
    expect(httpResponse.body.error).toBe(missingParamError('email').message);
  });
  ...
  test('Should return 400 if no password is provided', async () => {
    ...
    expect(httpResponse.statusCode).toBe(400);
    expect(httpResponse.body.error).toBe(missingParamError('password').message);
  });
  ...
  test('Should return 400 if an invalid no email is provided', async () => {
    ...
    expect(httpResponse.statusCode).toBe(400);
    expect(httpResponse.body.error).toBe(invalidParamError('email').message);
  });
  ...
  test('Should return 401 when invalid credentials are provided', async () => {
    ...
    expect(httpResponse.body.error).toBe(unauthorizedError().message);
  });
  ...
  test('Should return 500 if loginRouter is invalid dependencies are provided', () => {
    ...
    suts.forEach(async (sut) => {
      ...
      expect(httpResponse.body.error).toBe(serverError().message);
    });
  });
